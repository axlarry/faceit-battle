...
const BURST_LIMIT = 20; // allow up to 20 requests per window (generous for 35+ friends)
...
Deno.serve(async (req) => {
  ...
  try {
    const { nickname, endpoint = "stats" }: FaceitAnalyserRequest =
      await req.json();
    ...
    if (!response.ok) {
      console.error(
        `FaceitAnalyser API error: ${response.status} ${response.statusText}`,
      );
