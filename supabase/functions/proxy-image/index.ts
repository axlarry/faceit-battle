...
serve(async (req) => {
  ...
  try {
    ...
    if (!isAllowed) {
      console.log(
        `[proxy-image] Blocked request for non-allowed domain: ${parsedUrl.hostname}`,
      );
    ...
    console.log(
      `[proxy-image] Successfully fetched image, type: ${contentType}, size: ${imageData.byteLength}`,
    );
