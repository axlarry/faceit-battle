...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-discord-locale, x-discord-activity-storage-id, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version",
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "update_cache") {
      if (!cacheUpdateLimiter.allow(ip))
      ...
      // that APIs may return (e.g. "1234" or "1,234").
      ...
      if (p.cover_image !== undefined)
        patch.cover_image = p.cover_image ?? null;
      if (p.country !== undefined) patch.country = p.country ?? null;
      if (p.country_flag !== undefined)
        patch.country_flag = p.country_flag ?? null;
      if (p.region !== undefined) patch.region = p.region ?? null;
      if (p.region_ranking !== undefined)
        patch.region_ranking = safeInt(p.region_ranking);
      if (p.country_ranking !== undefined)
        patch.country_ranking = safeInt(p.country_ranking);
      ...
      if (error) {
        console.error(
          `[update_cache] DB error for ${p.player_id}:`,
          error.message,
          "| patch:",
    ...
    if (action === "add" || action === "update") {
      ...
      if (!p || !p.player_id || !p.nickname || !p.avatar)
        return json({ error: "Invalid player payload" }, 400);
    ...
    if (action === "migrate_auto") {
      ...
      const existingSet = new Set(
        (existingPublic || []).map((r: any) => r.player_id as string),
      );
      ...
      return json({
        migratedInserted: inserted,
        migratedUpdated: updated,
        sourceOwnerId,
      });
    }
    ...
    if (action === "refresh_all") {
      ...
      for (let i = 0; i < (friends || []).length; i += batchSize) {
        ...
        await Promise.all(
          batch.map(async (friend: any) => {
            try {
              // Use FACEIT official API to refresh elo/level (lcrypt.eu eliminated)
              const res = await fetch(
                `${Deno.env.get("SUPABASE_URL")}/functions/v1/proxy-faceit`,
                {
                  method: "POST",
                  headers: {
                  ...
                  body: JSON.stringify({
                    endpoint: `/players?nickname=${encodeURIComponent(friend.nickname)}`,
                  }),
                },
              );
              if (!res.ok) return;
              const d = await res.json();
              if (!d || !d.games?.cs2) return;
              const elo = d.games.cs2.faceit_elo ?? 0;
              const level = d.games.cs2.skill_level ?? 0;
    ...
    if (action === "update_nickname") {
      ...
      if (!playerId || !newNickname)
        return json({ error: "Missing playerId or newNickname" }, 400);
    ...
    if (action === "sync_nickname") {
      ...
      if (
        ...
        (!newAvatar || existing.avatar === newAvatar)
      ) {
