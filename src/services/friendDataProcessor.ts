  ...
  async updateFriendData(
    ...
    try {
      ...
      performanceMonitor.measureAsyncTime(
        `enriched-api-${friend.nickname}`,
        () => enrichedPlayerService.getEnrichedPlayerData(
          friend.nickname,
          friend.player_id,
          friend.country
        )
      ).catch(() => null).then(optimizedData => {
        debugLog(`Phase2 result:`, JSON.stringify(optimizedData).slice(0, 300));
        if (!optimizedData) {
  ...
  private async persistCache(
    ...
    const payload = {
      ...
      player: {
        ...
        region_ranking:  enriched?.region_ranking  != null ? Number(enriched.region_ranking)  : null,
        country_ranking: enriched?.country_ranking != null ? Number(enriched.country_ranking) : null,
      },
    };
