...
import {
  useStreamViewerTracking,
  useStreamViewerCount,
} from "@/hooks/useStreamViewerTracking";
...
export const LiveStreamPlayer = memo(
  ({ stream, isOpen, onClose }: LiveStreamPlayerProps) => {
    ...
    useStreamViewerTracking(
      stream?.nickname || null,
      isOpen && !!stream?.streamUrl,
    );
    const realtimeViewerCount = useStreamViewerCount(
      isOpen && stream?.nickname ? stream.nickname : null,
    );
    ...
    useEffect(() => {
      if (!isOpen || !stream?.streamUrl || !videoReady || !videoRef.current)
        return;

      console.log(
        "🎬 Initializing HLS player with DVR mode:",
        stream.streamUrl,
      );
    ...
    const seek = (seconds: number) => {
      if (videoRef.current) {
        const newTime = Math.max(
          0,
          Math.min(videoRef.current.currentTime + seconds, bufferEnd),
        );
