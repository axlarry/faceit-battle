...
import {
  groupLivePlayersByTeam,
  getPlayerTeamInfo,
} from "@/utils/teamGroupingUtils";
...
export const FriendsSectionHeader = React.memo(
  ({
    ...
    const soloPlayers = liveFriends.filter(
      (friend) =>
        !teams.some((team) =>
          team.players.some((p) => p.player_id === friend.player_id),
        ),
    );
                  ...
                  {teams.map((team) => (
                    <div key={team.id} className="flex -space-x-1 relative">
                      ...
                      {team.players.length > 3 && (
                        <div
                          className={`w-8 h-8 rounded-full border-2 ${team.color} bg-gray-700/50 flex items-center justify-center text-xs font-bold text-white ring-2 ring-opacity-30`}
                        >
                          +{team.players.length - 3}
                        </div>
                      )}
