...
export const FriendInfo = memo(
  ({ nickname, level, elo, lcryptData }: FriendInfoProps) => {
    ...
    const levelIcon = useMemo(
      () => LEVEL_ICONS[calculatedLevel] || LEVEL_ICONS[1],
      [calculatedLevel],
    );
            ...
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              {lcryptData?.region_ranking && (
                <span className="text-muted-foreground/70">
                  • EU #{lcryptData.region_ranking?.toLocaleString()}
                </span>
              )}
            </div>
          )}
          <TrendIndicator
            trend={lcryptData?.trend}
            report={lcryptData?.report}
          />
        </div>
