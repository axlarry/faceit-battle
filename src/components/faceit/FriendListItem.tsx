...
export const FriendListItem = React.memo(
  ({
    ...
    const handleWatchStreamClick = useCallback(
      (e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    const shouldShowLoadingOverlay =
      friend.lcryptData === undefined && isLoadingElo;
    ...
    return (
      ...
      >
        {/* Outer glow effect */}
        <div
          className={`pointer-events-none absolute inset-0 rounded-3xl ${playerStyles.glow} opacity-30 group-hover:opacity-60 transition-opacity duration-200 blur-md scale-100`}
        ></div>
        ...
        >
          ...
          <div className="absolute top-4 left-4 z-20">
            {index < 3 && (
              <div
                className={`absolute -bottom-1 -right-1 w-3 h-3 ${playerStyles.pulseColor} rounded-full animate-pulse shadow-lg`}
              >
                <div
                  className={`absolute inset-0 ${playerStyles.pulseColor} rounded-full animate-ping`}
                ></div>
              </div>
            )}
          </div>
            ...
            <div className="absolute inset-0 bg-slate-900/95 rounded-3xl flex items-center justify-center z-30 backdrop-blur-lg">
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  ...
                  <LoaderCircle
                    className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-orange-400 animate-pulse"
                    size={20}
                  />
                </div>
                <div className="text-center">
                  <span className="text-sm text-orange-400 font-semibold block">
                    Loading Player Data
                  </span>
