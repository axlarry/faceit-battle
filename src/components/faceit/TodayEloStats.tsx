...
export const TodayEloStats = ({ friends, isLoading }: TodayEloStatsProps) => {
  const stats = React.useMemo(() => {
    ...
    if (todaysElos.length === 0)
      return { avg: 0, count: 0, positive: 0, negative: 0, neutral: 0 };
    ...
    return (
      <div className="flex gap-2">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="bg-muted animate-pulse rounded-xl w-20 h-16"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Media ELO Today */}
      <div
        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border ${getAvgColor(stats.avg)} backdrop-blur-sm`}
      >
