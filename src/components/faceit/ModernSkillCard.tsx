...
export const ModernSkillCard = ({
  ...
  useEffect(() => {
    const loadFaceitAnalyserData = async () => {
      setIsLoading(true);
      try {
        const data = await faceitAnalyserService.getPlayerStats(
          player.nickname,
        );
  ...
  const currentLevel = lcryptData?.level
    ? parseInt(lcryptData.level)
    : lcryptData?.elo
      ? calculateLevelFromElo(lcryptData.elo)
      : player.elo
        ? calculateLevelFromElo(player.elo)
        : player.level || 1;
  ...
  const faRating = faceitAnalyserData
    ? Math.round(
        faceitAnalyserData.avg_hltv * 100 * (faceitAnalyserData.wr / 100) * 10,
      ) / 10
    : null;
  ...
  return (
    <div className="relative group">
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-r ${getLevelGradient(currentLevel)} rounded-2xl blur-xl opacity-20 group-hover:opacity-40 transition-all duration-500`}
      />
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-lg p-2 border border-purple-500/20">
                <div className="text-purple-400 font-bold">
                  {faceitAnalyserData.avg_hltv?.toFixed(2) || "N/A"}
                </div>
              ...
              <div className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-lg p-2 border border-cyan-500/20">
                <div className="text-cyan-400 font-bold">
                  {faceitAnalyserData.avg_krr?.toFixed(2) || "N/A"}
                </div>
              ...
              <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-lg p-2 border border-green-500/20">
                <div className="text-green-400 font-bold">
                  {faceitAnalyserData.avg_k?.toFixed(2) || "N/A"}
                </div>
              ...
              <div className="bg-gradient-to-r from-blue-500/10 to-indigo-500/10 rounded-lg p-2 border border-blue-500/20">
                <div className="text-blue-400 font-bold">
                  {faceitAnalyserData.k?.toLocaleString() || "N/A"}
                </div>
