...
import { parseMatchReport, findMatchEloChangeFromReport } from "@/utils/matchReportParser";
...
export const MatchRow = ({
  ...
  const matchReports = lcryptData?.report ? parseMatchReport(lcryptData.report) : [];
  const matchEloChange = findMatchEloChangeFromReport(match, matchReports, matchIndex, player);
  ...
  return <TableRow key={match.match_id} className={`border-white/10 hover:bg-white/10 transition-colors cursor-pointer ${isWin === true ? 'bg-green-500/5' : isWin === false ? 'bg-red-500/5' : ''}`} onClick={() => onMatchClick(match)}>
      ...
      <TableCell>
        {matchEloChange !== null ? <div className="flex items-center gap-1">
            {matchEloChange > 0 ? <TrendingUp className="w-4 h-4 text-green-400" /> : matchEloChange < 0 ? <TrendingDown className="w-4 h-4 text-red-400" /> : <Minus className="w-4 h-4 text-gray-400" />}
            <span className={`font-bold ${matchEloChange > 0 ? 'text-green-400' : matchEloChange < 0 ? 'text-red-400' : 'text-gray-400'}`}>
              {matchEloChange > 0 ? '+' : ''}{matchEloChange}
            </span>
