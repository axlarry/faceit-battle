...
export const getEloChange = (
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (rankKeys.includes(key.toLowerCase())) {
        if (Array.isArray(value)) {
          const playerRankData = value.find(
            (item: any) => item && item.player_id === player.player_id,
          );
          if (playerRankData) {
          ...
          for (const prop of allRankProps) {
            if ((value as any)[prop] !== undefined) {
              ...
              if (typeof rankValue === "number") {
                if (
                  prop.toLowerCase().includes("change") ||
                  prop.toLowerCase().includes("diff") ||
                  prop.toLowerCase().includes("delta")
                ) {
                ...
                if ("after" in rankValue && "before" in rankValue) {
                  const after =
                    typeof rankValue.after === "string"
                      ? parseInt(rankValue.after)
                      : rankValue.after;
                  const before =
                    typeof rankValue.before === "string"
                      ? parseInt(rankValue.before)
                      : rankValue.before;
                ...
                if ("change" in rankValue) {
                  const change =
                    typeof rankValue.change === "string"
                      ? parseInt(rankValue.change)
                      : rankValue.change;
  ...
  for (const section of rankSections) {
    if (matchStatsData[section]) {
      if (Array.isArray(matchStatsData[section])) {
        const playerRankData = matchStatsData[section].find(
          (item: any) => item && item.player_id === player.player_id,
        );
        if (playerRankData) {
  ...
  if (matchStatsData.elo_change && Array.isArray(matchStatsData.elo_change)) {
    const playerEloData = matchStatsData.elo_change.find(
      (elo: any) => elo.player_id === player.player_id,
    );
